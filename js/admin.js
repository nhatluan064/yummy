...
class YummyAdmin {
  ...
  async loadDashboardData() {
    try {
      ...
      if (tablesResult.success) {
        const occupiedTables = tablesResult.data.filter(
          (table) => table.status === "occupied"
        );
        document.getElementById("tables-occupied").textContent =
          occupiedTables.length;
      }

      if (menuResult.success) {
        const activeItems = menuResult.data.filter(
          (item) => item.isActive !== false
        );
        document.getElementById("menu-items-active").textContent =
          activeItems.length;
      }
    } catch (error) {
    ...
    const menuItemsHtml = items
      ...
      <div class="menu-item-card" data-id="${item.id}">
        <div class="item-image">
          <img src="${
            item.image || this.getPlaceholderImage(item.name)
          }" alt="${item.name}" 
               onerror="this.src='https://placehold.co/300x200/e9a28a/FFFFFF?text=${encodeURIComponent(
                 item.name
               )}'">
        ...
        <div class="item-actions">
          <button class="btn btn-sm btn-secondary" onclick="yummyAdmin.editMenuItem('${
            item.id
          }')">
            <i class="fa-solid fa-edit"></i> Sửa
          </button>
          <button class="btn btn-sm btn-danger" onclick="yummyAdmin.deleteMenuItem('${
            item.id
          }', '${item.name}')">
            <i class="fa-solid fa-trash"></i> Xóa
          </button>
          <button class="btn btn-sm ${
            item.isActive !== false ? "btn-warning" : "btn-success"
          }" 
                  onclick="yummyAdmin.toggleMenuItemStatus('${item.id}', ${
          item.isActive !== false
        })">
            <i class="fa-solid fa-${
              item.isActive !== false ? "pause" : "play"
            }"></i> 
  ...
  updateMenuStats(items) {
    ...
    document.getElementById("avg-menu-price").textContent =
      this.formatCurrency(avgPrice);
  }
  ...
  async handleMenuItemSubmit(e) {
    ...
    try {
      ...
      if (this.currentEditItemId) {
        result = await this.firebaseService.updateMenuItem(
  ...
  async toggleMenuItemStatus(itemId, currentStatus) {
    try {
      ...
      if (result.success) {
        this.showSuccess(
          `${!currentStatus ? "Bật" : "Tắt"} bán món ăn thành công!`
        );
        await this.loadMenuItems();
      } else {
        this.showError(result.error || "Có lỗi xảy ra");
    ...
    const tablesHtml = tables
      ...
      <div class="table-card ${table.status}" data-id="${table.id}">
        ...
        <div class="table-actions">
          <button class="btn btn-sm btn-primary" onclick="yummyAdmin.updateTableStatus('${
            table.id
          }', 'available')">
            <i class="fa-solid fa-unlock"></i> Mở bàn
          </button>
          <button class="btn btn-sm btn-warning" onclick="yummyAdmin.updateTableStatus('${
            table.id
          }', 'occupied')">
  ...
  async updateTableStatus(tableId, status) {
    try {
      const result = await this.firebaseService.updateTableStatus(
        tableId,
        status
      );
      if (result.success) {
    ...
    const ordersHtml = orders
      ...
      <div class="order-card ${order.status}" data-id="${order.id}">
        <div class="order-header">
          ...
          <div class="order-status ${order.status}">${this.getOrderStatusText(
          order.status
        )}</div>
        </div>
  ...
  async initializeSampleData() {
    if (
      !confirm(
        "Bạn có chắc muốn khởi tạo dữ liệu mẫu? Thao tác này sẽ thêm menu và bàn mẫu."
      )
    ) {
      return;
    }
