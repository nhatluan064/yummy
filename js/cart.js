...
class CartManager {
  ...
  renderMenu() {
    ...
    Object.entries(categories).forEach(([category, items]) => {
      ...
      items.forEach(item => {
        ...
        menuCard.className = `menu-card ${item.status === 'soldout' ? 'soldout' : ''}`;
          ...
          <div class="menu-item-info">
            ...
            <div class="item-status">
              ${item.status === 'available' ? 
                `<span class="available">Còn món (${item.inventory || 0})</span>` : 
  ...
  checkout() {
    ...
    if (this.orderType === 'delivery') {
      ...
      if (!address || !phone) {
        Utils.showNotification("Vui lòng điền đầy đủ thông tin giao hàng!", 'error');
        return;
      }
    ...
    const orderTypeText = this.orderType === 'dine-in' ? 
      `tại bàn ${this.selectedTable}` : 'giao hàng';
  ...
  sendToKitchen(orderData) {
    ...
    const kitchenItems = Object.entries(this.cart).map(([itemId, cartItem]) => {
      ...
      const menuItem = this.menuItems.find(item => item.id === itemId || item.name === itemId);
    ...
    const kitchenOrder = {
      ...
      orderType: this.orderType === 'dine-in' ? 'Tại bàn' : 
                  this.orderType === 'delivery' ? 'Giao hàng' : 'Mang về',
      items: kitchenItems,
      total: orderData.total,
      customerInfo: this.orderType === 'delivery' ? {
