...
class SharedMenuManager {
  ...
  getAvailableItems() {
    return this.getMenuData().filter(
  ...
  getMenuStats() {
    ...
    return {
      ...
      availableItems: menuData.filter((item) => item.status === "available")
        .length,
      soldOutItems: menuData.filter((item) => item.status === "soldout").length,
      todaySales: menuData.reduce(
        (sum, item) => sum + (item.soldToday || 0),
        0
      ),
      totalRevenue: menuData.reduce(
        (sum, item) => sum + (item.soldToday || 0) * item.price,
        0
      ),
    };
  }
