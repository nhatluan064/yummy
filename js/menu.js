...
class MenuManager {
      ...
      toggleAdminBtn.addEventListener("click", () => {
        ...
        if (isHidden) {
          ...
          toggleAdminBtn.innerHTML =
            '<i class="fas fa-eye-slash"></i> Ẩn Quản Lý';
        } else {
  ...
  renderStats() {
    ...
    const availableItems = this.menuItems.filter(
      (item) => item.status === "available"
    ).length;
    const soldOutItems = this.menuItems.filter(
      (item) => item.status === "soldout"
    ).length;
    const todaySales = this.menuItems.reduce(
      (sum, item) => sum + (item.soldToday || 0),
      0
    );
  ...
  createItemElement(item) {
    ...
    return `
      ...
      item.name
    }" class="item-image" onerror="this.src='https://placehold.co/300x200/FF6B35/FFFFFF?text=No+Image'">
                
                <div class="item-header">
                    <h4 class="item-name">${item.name}</h4>
                    <span class="item-price">${Utils.formatCurrency(
                      item.price
                    )}</span>
                </div>
                ...
                <div class="item-actions">
                    <div class="item-status">
                        ...
                        ${
                          item.inventory !== undefined
                            ? `<span class="item-inventory">(Còn: ${item.inventory})</span>`
                            : ""
                        }
                    </div>
                    ...
                    <div class="item-buttons">
                        ...
                        ${
                          isAdmin
                            ? `
                            <button class="item-btn edit" onclick="menuManager.showEditModal('${
                              item.id
                            }')">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="item-btn toggle-status" onclick="menuManager.toggleItemStatus('${
                              item.id
                            }')">
                            ...
                            <button class="item-btn delete" onclick="menuManager.deleteItem('${
                              item.id
                            }')">
  ...
  handleAddItem(formData) {
    const newItem = {
      ...
      image:
        formData.get("itemImage") ||
        "https://placehold.co/300x200/FF6B35/FFFFFF?text=No+Image",
      status: "available",
  ...
  toggleItemStatus(itemId) {
    ...
    Utils.showNotification(
      `Đã chuyển "${item.name}" thành ${statusText}`,
      "success"
    );
  }
        ...
        (item) => `
            <div class="bulk-item">
                ...
                <label for="bulk-${item.id}" class="bulk-item-info">
                    ...
                    <div class="bulk-item-category">${item.category} - ${
  ...
  handleBulkUpdate() {
    const selectedItems = Array.from(
      document.querySelectorAll("#bulkItemsList input:checked")
    ...
    Utils.showNotification(
      `Đã cập nhật ${selectedItems.length} món ăn`,
      "success"
    );
  }
  ...
  renderQuickOrder() {
    ...
    if (this.quickOrder.length === 0) {
      container.innerHTML =
        '<div class="text-center text-muted">Chưa có món nào được chọn</div>';
      totalElement.innerHTML = "<strong>Tổng: 0₫</strong>";
      return;
    }

    container.innerHTML = this.quickOrder
      .map(
        (order) => `
            <div class="quick-order-item">
                <div class="quick-item-info">
                    <div class="quick-item-name">${order.name}</div>
                    <div class="quick-item-price">${Utils.formatCurrency(
                      order.price
                    )}</div>
                </div>
                <div class="quick-item-controls">
                    <button class="btn btn-sm btn-outline-secondary" onclick="menuManager.updateQuickOrderQuantity('${
                      order.id
                    }', ${order.quantity - 1})">-</button>
                    <span class="mx-2">${order.quantity}</span>
                    <button class="btn btn-sm btn-outline-secondary" onclick="menuManager.updateQuickOrderQuantity('${
                      order.id
                    }', ${order.quantity + 1})">+</button>
                    <button class="btn btn-sm btn-danger ms-2" onclick="menuManager.removeFromQuickOrder('${
                      order.id
                    }')">
                        <i class="fas fa-trash"></i>
                    </button>
    ...
    const total = this.quickOrder.reduce(
      (sum, order) => sum + order.price * order.quantity,
      0
    );
    totalElement.innerHTML = `<strong>Tổng: ${Utils.formatCurrency(
      total
    )}</strong>
  ...
  orderByCategory(category) {
    ...
    if (categoryItems.length === 0) {
      Utils.showNotification(
        `Không có món nào trong danh mục ${category}`,
        "warning"
      );
      return;
    }
