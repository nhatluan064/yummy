...
class KitchenManager {
  ...
  generateDemoOrders() {
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      demoOrders.push({
        id: `order-${Date.now()}-${i}`,
        tableNumber:
          orderType === "Tại bàn" ? Math.floor(Math.random() * 20) + 1 : null,
        ...
        startTime:
          status !== "pending"
            ? new Date(orderTime.getTime() + 5 * 60 * 1000).toISOString()
            : null,
        completeTime:
          status === "ready"
            ? new Date(orderTime.getTime() + 20 * 60 * 1000).toISOString()
            : null,
      });
    }
  ...
  renderOrders() {
    ...
    // Render order cards
    ordersGrid.innerHTML = sortedOrders
  ...
  createOrderCard(order) {
    ...
    const displayTime = orderTime.toLocaleTimeString("vi-VN", {
      hour: "2-digit",
      minute: "2-digit",
    });
            ...
            <div class="order-card ${order.status}" data-order-id="${order.id}">
                <div class="order-header">
                    ...
                    <div class="order-status">
                        ...
                        <div class="order-timer" data-order-time="${
                          order.orderTime
                        }">
                            ${timeElapsed}
                        </div>
    ...
    orderDetailBody.innerHTML = `
            <div class="order-detail">
                <div class="detail-section">
                    ...
                    <p><strong>Thời gian đặt:</strong> ${orderTime.toLocaleString(
                      "vi-VN"
                    )}</p>
                    <p><strong>Trạng thái:</strong> ${this.getStatusText(
                      order.status
                    )}</p>
                    ${
                      order.customerInfo
                ...
                <div class="detail-section">
                    ...
                    ${Object.entries(this.groupItemsByCategory(order.items))
                      .map(
                        ([category, items]) => `
                ...
                <div class="detail-section">
                    <h4>Thời gian</h4>
                    <p><strong>Đặt hàng:</strong> ${orderTime.toLocaleString(
                      "vi-VN"
                    )}</p>
                    ${
                      order.startTime
                        ? `<p><strong>Bắt đầu làm:</strong> ${new Date(
                            order.startTime
                    ...
                    ${
                      order.readyTime
                        ? `<p><strong>Sẵn sàng:</strong> ${new Date(
                            order.readyTime
                          ).toLocaleString("vi-VN")}</p>`
                        : ""
                    }
                    ${
                      order.completeTime
                        ? `<p><strong>Hoàn thành:</strong> ${new Date(
                            order.completeTime
                          ).toLocaleString("vi-VN")}</p>`
                        : ""
                    }
  ...
  updateStats() {
    ...
    const cookingCount = this.orders.filter(
      (o) => o.status === "cooking"
    ).length;
