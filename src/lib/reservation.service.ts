...
class ReservationService extends FirestoreService<TableReservation> {
  ...
  async createReservation(data: Omit<TableReservation, 'id' | 'createdAt' | 'updatedAt' | 'status'>) {
    if (!data.customerName || !data.customerPhone) throw new Error('Thiếu thông tin khách hàng');
    if (data.numberOfGuests < 1) throw new Error('Số khách không hợp lệ');
    return this.create({ ...data, status: 'pending' } as TableReservation);
  }

  async confirm(id: string) { return this.update(id, { status: 'confirmed' }); }
  async cancel(id: string) { return this.update(id, { status: 'cancelled' }); }
